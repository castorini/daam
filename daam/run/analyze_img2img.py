...
def main():
    ...
    with torch.cuda.amp.autocast(dtype=torch.float16):
        ...
        num_gen = 0
        im = cv2.imread(args.input_image, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255
        im = torch.from_numpy(im).unsqueeze(0)
        im = im.permute(0, 3, 1, 2)
        im = F.interpolate(im, size=(512, 512), mode="bicubic", align_corners=False)
        ...
        while True:
            ...
            try:
                ...
                with autocast("cuda"):
                    ...
                    clear_heat_maps()
                    plt.clf()
                    out = pipe(prompt, init_image=im, num_inference_steps=2, strength=0, guidance_scale=7.5)#, do_intermediates=args.save_intermediates)
                    of = output_folder / id
